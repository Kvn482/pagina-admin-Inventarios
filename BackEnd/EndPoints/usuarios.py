...
@router.get("/verUsers/", response_model=List[page_schemas.Usuarios])
...
def create_user(entrada:page_schemas.RegisterUser,db:session=Depends(get_Users)):
   ...
   usuario = page_models.Usuarios(nickname = entrada.nickname, correo = entrada.correo, contrasena = hashed_password, estado = 1,codigo = codigo1)
   db.add(usuario)
   db.commit()
   db.refresh(usuario)
   return usuario
